...
public class JDateChooser extends JPanel implements ChangeListener, ItemListener {
    ...
    private JComboBox month = new JComboBox(new String[] { "一月", "二月", "三月", "四月", "五月", "六月", "七月", "八月", "九月", "十月",
            "十一月", "十二月" });
    ...
    public JDateChooser(Calendar initialDate) {
        ...
        p1.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createTitledBorder(" 日期"), BorderFactory
        ...
        p2.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createTitledBorder(" 时间"), BorderFactory
                .createEmptyBorder(0, 10, 10, 10)));
        // time.setPreferredSize(new Dimension(0, 20));
        JSpinner.DateEditor editor = new JSpinner.DateEditor(time, "HH:mm:ss.SSS");
     ...
     */
    public static Calendar showDialog(Component c, String title, Calendar initialDate) {
        ...
        p.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createEmptyBorder(6, 6, 0, 6), BorderFactory
                .createEtchedBorder()));
        p.add(dc);
        dlg.add(p);
    ...
    private static class Clock extends JComponent implements ChangeListener {
        ...
        protected void paintComponent(Graphics g) {
            ...
            float ma = (float) Math.PI * 2 / 60 * (minute + (float) second / 60);
        ...
        private void drawHand(Graphics2D g2, float ox, float oy, float a, float len, boolean w) {
    ...
    public static void main(String[] args) throws Exception {
        ...
        JFrame f = new JFrame();
        f.setVisible(true);
        Calendar c = JDateChooser.showDialog(f, "请选择日期", null);
        if (c == null)
            JOptionPane.showMessageDialog(null, "你没有选择时间");
        else
            JOptionPane.showMessageDialog(null, "你选择的时间是" + c.getTime());
    }
}
