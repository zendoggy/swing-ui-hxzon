...
/**
 * Demoed Component: {@link WizardDialog} <br> Required jar files: jide-common.jar, jide-components.jar,
 * jide-dialogs.jar <br> Required L&F: Any L&F
 */
public class WizardDemo extends AbstractDemo {
    ...
    public String getDescription() {
        return "Wizard is a well-known user interface that is ideal to guide user through for complex and unfamiliar tasks. A typical usage of it is project wizard - which asks user a couple questions and generate source code of a project automatically for user.\n" +
                ...
                "\n" +
                "Demoed classes:\n" +
                "com.jidesoft.wizard.WizardDialog\n" +
    ...
    public Component getDemoPanel() {
        ...
        buttonPanel.addButton(new JButton(new AbstractAction("Wizard 97 Style") {
            public void actionPerformed(ActionEvent e) {
                int style = WizardStyle.WIZARD97_STYLE;
                Container topLevelAncestor = buttonPanel.getTopLevelAncestor();
                showWizardDemo(topLevelAncestor instanceof Frame ? (Frame) topLevelAncestor : null, false, style);
            }
        }));

        buttonPanel.addButton(new JButton(new AbstractAction("Java Style") {
            public void actionPerformed(ActionEvent e) {
        ...
        buttonPanel.addButton(new JButton(new AbstractAction("Mac OS X Style") {
            public void actionPerformed(ActionEvent e) {
                int style = WizardStyle.MACOSX_STYLE;
                Container topLevelAncestor = buttonPanel.getTopLevelAncestor();
                showWizardDemo(topLevelAncestor instanceof Frame ? (Frame) topLevelAncestor : null, false, style);
            }
        }));
        ...
        private void updateNextButton() {
            if (_button1 != null && _button2 != null) {
                if (_button1.isSelected()) {
                    ...
                    fireButtonEvent(ButtonEvent.CHANGE_BUTTON_FOCUS, ButtonNames.NEXT);
                    fireButtonEvent(ButtonEvent.SET_DEFAULT_BUTTON, ButtonNames.NEXT);
                }
                else if (_button2.isSelected()) {
                    fireButtonEvent(ButtonEvent.DISABLE_BUTTON, ButtonNames.NEXT);
                    fireButtonEvent(ButtonEvent.SET_DEFAULT_BUTTON, ButtonNames.BACK);
                }
            }
        }
